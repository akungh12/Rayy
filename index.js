...
// --- API BARU: UPDATE KATEGORI ---
app.post('/api/category/update', async (req, res) => {
    try {
        const { oldName, newName, image } = req.body;
        if(!oldName || !newName) return res.status(400).json({ status: 'error', message: 'Nama lama dan baru wajib diisi' });
        ...
        // Jika nama berubah, lakukan rename key
        if(oldName !== newName) {
            // Cek nama baru sudah ada atau belum
            if(data.categories[newName]) {
                return res.status(400).json({ status: 'error', message: 'Nama kategori baru sudah ada' });
            }
            data.categories[newName] = data.categories[oldName];
            delete data.categories[oldName];
        }

        // Simpan data gambar kategori (jika ada)
        if(!data.category_images) data.category_images = {};
        if(image) {
            data.category_images[newName] = image;
        }

        // Hapus data gambar lama jika nama berubah
        if(oldName !== newName && data.category_images[oldName]) {
            delete data.category_images[oldName];
        }

        await updateGitHubData(data, current.sha);
        res.json({ status: 'success', message: 'Kategori berhasil diupdate', data: data });

    } catch (error) {
        console.error("Gagal update kategori:", error);
        res.status(500).json({ status: 'error', message: 'Gagal update kategori' });
    }
});
